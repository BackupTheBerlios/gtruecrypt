...
pygtk.require('2.0')
import gtk
"""importing classes from the TrueCrypt-Wrapper made by Jens Kadenbach"""
...
class main_window:
    ...
    def table(self):
        ...
        self.TC = TrueCrypt(self.get_sudo()) # FIXME sudo_passwd?!?
    ...
    def button_change(self, treeview):
        ...
        if iter:
            ...
            if state[0] == tcerr.umounted:
                self.button.set_label("mount")
            elif state[0] == tcerr.not_found:
                self.button.set_label("details")
            elif state[0] == tcerr.mounted:
                self.button.set_label("unmount")
            elif state[0] == tcerr.unknown_error:
                self.button.set_label("details")
    ...
    def action(self, button):
        ...
        if iter:
            ...
            if state[0] == tcerr.umounted:
                ...
                print self.TC.mount(0, target[0], self.get_sudo())
            elif state[0] == tcerr.mounted:
                self.liststore.set(iter, 2, "unmounted")

    def get_sudo(self):
        pop = Popup(type='sudo_request')
        return pop.get_sudo()
...
class Popup:
    """A Popup to retrieve the sudo password"""
    def __init__(self, type, text=None):
        ...
        self.__window.set_title(self.__get_title())
        self.__window.set_size_request(200, 100)
        self.__window.connect("delete_event", self.__window.destroy)

        self.vbox = gtk.VBox(False, 0)
        self.__window.add(self.vbox)

        self.__create_type_special()

    def __get_title(self):
        if self.type == 'sudo_request':
    ...
    def __create_type_special(self):
        if self.type == 'sudo_request':
            self.entry = gtk.Entry(max=255) # FIXME self.enter_callback needed!!!
